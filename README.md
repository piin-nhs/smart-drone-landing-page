...
## 🚀 HƯỚNG DẪN CÀI ĐẶT & CHẠY DỰ ÁN DƯỚI LOCAL
...
Tạo file `.env.local` ở thư mục gốc và điền đầy đủ các thông tin cấu hình tương tự như sau:
```env
MONGODB_URI=mongodb+srv://<username>:<password>@cluster.mongodb.net/helicorp-drone
OPENROUTER_API_KEY=sk-or-v1-xxxxxxxxxxxxxxxxxxxx
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/...
```

### 4. Chạy dự án ở chế độ Development
Chạy lệnh dưới đây để khởi chạy local server:
```bash
npm run dev
```
Mở trình duyệt và truy cập [http://localhost:3000](http://localhost:3000) để trải nghiệm website.

### 5. Kiểm tra TypeScript & Build Dự Án
Để kiểm tra lỗi Type và build sản phẩm cho Production:
```bash
npx tsc --noEmit
npm run build
```
...
- Website: [https://helicorp.vn](https://helicorp.vn)
- Vị trí: Thực Tập Sinh IT Phát Triển Website
